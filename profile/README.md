## A possible answer to the question " How education system would look like, if students were the primary stakeholder of the system instead of institutions & teachers?"

### Rethinking education, Revisiting vedic methodologies with modern blend.
> “Where there is righteousness in the heart, there is beauty in the character. When there is beauty in the character, there is harmony in the home. When there is harmony in the home, there is order in the nation. When there is order in the nation, there is peace in the world.”<br>
> – A.P.J. Abdul Kalam

# Post GPT Era Education Framework!

## Curriculum

### **The Process**
Somewhat equivalent of admission process. 

It'll be completely merit based through three gamified platforms.
There won't be any annual admission tests but rather it'll be throughout the year. 
The students who'll top on the leaderboard in those platforms will meet the eligibility criteria.

> Short description of those three platform:
### Ganit (Computation)
This platform is dedicated to test the mental abilities (IQ) of the child.

Areas of focus:
- Practical Mathematics
- Pattern Recognition
- General Intelligence (Logic, Reasoning & Planning)

### Shiksha (Humanity)
This platform is dedicated to test the linguistic ability (EQ) of the child.

Areas of focus:
- Textual Comprehension (Reading)
- Typing (Writing & Effective Articulation)
- Audio-visual Comprehension (Speech & Videos)
- Basic Programming (Harvard CS50 by David J Malan)
- Language
    - Prakrit (Mother Tongue/Native/vernacular language.)
    - Sanskrit ( Link language for precise expression, learning, discussions & debate )
    - English ( Current global standard for language)
    - Python ( Easy to learn, high level programming language for Algorithm development )
    - Javascript ( Easy to learn, high level programming language for Interface development )

### Veda (General Knowledge)
This platform is dedicated to test the retention & analytical ability of the child.

Areas of focus:
- Critical Thinking.
- First Principle Thinking
- Zeroth Principle Thinking
- Health & Fitness.
- Big bang & basic knowledge of solar system, earth.
- Basic Physics, Chemistry & Biology (CBSE 12th Standard)
- Evolution
- Human history
- Mythology & religions.
- Continents, countries & Belief systems
- Capitalism
- Human values & Differnt ways of living
- Discovary & Inventions
- Basic Knowledge of
    - Dharmashastra
    - Arthashastra
    - Karmashastra
    - Sevashastra

#### Guidelines for using this Platforms.

There will be multiple games to play.
Play any games to score points & rank in leaderboard.
Upon scoring in leaderboard, you'll win rings as achievements.

You can also join tournaments to compete with other students & win real prizes.

### **The Programs**
Somewhat equivalent of classes.

It'll be based on skills and ability of the student.
It is structured in such a way that, it'll act as a mirror to you life & represent phases of your life.

> Short description of the programs:
### Literacy
The first program will be conducted on the virtual Maverick Mandir Campus & it'll majorly focus on getting digital/future literacy.
It is structured in such a way that students accustomed to different ways can integrate to maverick way of doing things & experiment for themselves wheather it is suited for them or not without any commitments.

Areas of focus:
- Health Literacy (Sleep, Diet, Workout, Biomarkers)
- Financial Literacy. ( Personal finance, How money works)
- Cultural Literacy. (Develop resistance to Indoctrination/Manipulation, Know your culture better)
- Programming Literacy. ( Content | Design | Interface | Algorithm Development)
- Self Authoring & Future (Direction) Planning.
- How to Learn(Best learning Methods)
- Verbal & Non-verbal Communication
- Mentor pre-program students.

### Generalist
The second propram. Learning & Studying everything that made human civilization possible from the POV that, if human civilization were to collapse for x reason, You'll be able restart the civilization on a small scale at least within a century.

Also you will visit Physical Maverick Mandir Campus. This program is basically apprenticeship.

Areas of focus: 
- Intuition Development
- Time Management
- Money Management
- Bandwidth Management
- Persuation & Selling
- Investing

Beyond this program, there won't be any generic paths, Students will choose their own path.

### Specialist
You focus on one single thing & along with like minded people, you startup or join one.
Or you join the military/beurocracy.

You'll lose your membership, if you choose something that is not an acceptable career option in maverick charter.

There is no such thing as failure as endgame, failure is a process. Endgame is either Giving Up(Voluntary or involuntary) or Succed.

You'll stay full time at Physical Maverick Mandir campus in order to spend more time with your teammates to brainstrom or build the product or service you're building.

### Independence
You made a decent amount of money & build an investment portfolio along the way that generates enough cashflow to sustain you family.

In current scenario: You need around 2.4 crore lumpsum in India to retire.

Retire: You no longer have to sacrifice your present for your future.

### Polymath
Now you can start your riskier ventures, maybe you could get into politics.

### **The Protocols**
Somewhat equivalent of routines.

It is a flexible yet firm proposal to govern yourself effectively.

It is structured from a quarterly/annualy POV instead of daily routines, So it is flexible.
It has specific amount of time dedicated to certain activities, So it is firm.

Idea is to average out the time spend on an individual protocol & map it to an optimal daily limits.

So, we'll set a limit with second precision per day on average. A day/earth rotation is equavalent to `86164 seconds`.

Left: 8164 sec
### Health

#### Sleep
###### 29000s/29ks (8:5 HR)

#### Workout 

##### 4ks (1:7 HR)

#### Diet
##### 

#### Aesthetics
##### 

### Learn
Learning through form such as `Reading`, `Podcasts`, `Videos`, `Discussion`, `Debate `, `Monologue `, `Dialogue`, `Polylogue`, `Playground` or any other form of learning.

One learning session is 1500s/1.5 ks (0:25 HR) long, followed by a 500s (0:8 HR) of idle break to think through & recall the learnings.

##### 6 such learning sessions per day on average.


### Policy

Optimal: One learning session equivalent.

#### Journal
#### Planning


### Social

#### Productive

If the activities you're doing, thing you're building or the problem you're solving along with your peers are dedicated to benefit others. It is a productive social act.

##### 29000s/29ks (8:5 HR)

#### Unproductive
If the activities you're doing, thing you're building or the problem you're solving to benefit yourself or for your amusement, It is an unproductive social act.

##### 4ks (1:7 HR)

#### Chore
The activities that are nor productive neither unproductive, it is necessary but serves no purpose. Activities that you're doing is just for the sake of doing it.

Example: when your mother tells you do your bed, you have to do it, there is no other way!